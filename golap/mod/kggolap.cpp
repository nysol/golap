...
// Grannu test NewVERISOM
/*
Result kgmod::Enum_NEW(QueryParams& query, Ewah& dimBmp ,size_t tlimit=45) {
  ...
  unordered_map<size_t, size_t> itemFreq;
	...
	// node granularity なしのみ
	if ( !isNodeGranu ) {

	// ["field name"] -> item bitmap
  unordered_map<string, Ewah> ex_occ_cacheOnceQuery;      

  // [vnodes] -> exists(count)
  unordered_map<vector<string>, size_t, boost::hash<vector<string>>> checked_node2;  
    
	// CHECK 用
	cerr << "icnt "  << query.itemFilter.numberOfOnes() << endl;
	size_t icnt =query.itemFilter.numberOfOnes();
	int cnt=1;

	// 枝刈のみ先にしてフィルターした方がいいかも？
	for (auto i2 = query.itemFilter.begin(), ei2 = query.itemFilter.end(); i2 != ei2; i2++) {
    ...
    vector<string> vnode2;
    string node2 = mt_occ->itemAtt->item[(*i2)];
		vnode2.resize(1);
		vnode2[0] = node2;

		//ここ変更 node gruがセットされてる時はセット要
    if (itemFreq.find(*i2) == itemFreq.end()) {
        ...
        if (isTraGranu) {
          itemFreq[*i2] = mt_occ->attFreq(query.granularity.second, vnode2,
				...
				checked_node2[vnode2] = itemFreq[*i2];
		  }
			// node granularity なしでここは通らない
			else { continue;}
				// node  granularity なしでここは通らない
		}else{ continue; }
    ...
    for (auto i1 = coitems.begin(), ei1 = coitems.end(); i1 != ei1; i1++) {
      ...
      if (pmi < query.selCond.minPMI) continue;
			...
			float skey;
			if (query.sortKey == SORT_SUP)       { skey = -sup; } 
			else if (query.sortKey == SORT_CONF) { skey = -conf1;} 
			else if (query.sortKey == SORT_LIFT) { skey = -lift; } 
			else if (query.sortKey == SORT_JAC)  { skey = -jac; }
			else if (query.sortKey == SORT_PMI)  { skey = -pmi; } 
			else {  notSort = true;  skey = -10; }
    ...
    if (itemFreq.find(*i2) == itemFreq.end()) {
        ...
        if (isTraGranu) {
          itemFreq[*i2] = mt_occ->attFreq(query.granularity.second, vnode2,
				...
				checked_node2[vnode2] = itemFreq[*i2];
				itemInTheAtt2 = mt_occ->itemAtt->bmpList.GetVal(query.granularity.second, vnode2);
				itemInTheAtt2 = itemInTheAtt2 & query.itemFilter;

        if (isNodeGranu) {
        	// これの速度確認要
					for (auto at2 = itemInTheAtt2.begin(), eat2 = itemInTheAtt2.end(); at2 != eat2; at2++) {
						itemFreq[*at2] = checked_node2[vnode2];
					}
        }
		  }
			// node granularity なしでここは通らない
  ...
  unordered_map<vector<string>, bool, boost::hash<vector<string>>> checked_node2_b;  
      ...
      vnode2 = mt_occ->itemAtt->key2att(*i2, query.granularity.second);
        ...
        if (checked_node1.find({node1, traAtt}) == checked_node1.end()) {
          checked_node1[{node1, traAtt}] = true;
				...
				//cerr << *(itemInTheAtt1.begin()) << " vs " << *(itemInTheAtt2.begin()) << " : " << *i1 << " vs " << *i2 << endl;
        if (*(itemInTheAtt1.begin()) >= *(itemInTheAtt2.begin())) continue;
        ...
        //if (*i1 >= *i2) break;
          ...
          itemNo4node = itemNo4node_map[vnode1];
				...
				//cerr << "runi1x " << itemInTheAtt1.numberOfOnes() << endl;
				///
				//for (auto at1 = itemInTheAtt1.begin(), eat1 = itemInTheAtt1.end(); at1 != eat1; at1++) {
				//	if (isTimeOut) {stat = 2; break;}
				//	size_t itemNo4node;
        ...
        //     itemNo4node = *i1;
        //  }
        //  //cerr << "add pair " << traAtt << " " << node2 << " " << node1 << " " << itemNo4node << endl;
				//	coitems[itemNo4node]++;
     	 	//}
     }
		}
	...
	//else{
//
//		cerr << "tcnt2 "  << traNum << endl;
//		
//    unordered_map<string, Ewah> ex_occ_cacheOnceQuery;      // ["field name"] -> item bitmap
//
    //unordered_map<vector<string>, bool, boost::hash<vector<string>>> checked_node2;  // [vnodes] -> exists
//    unordered_map<vector<string>, size_t, boost::hash<vector<string>>> checked_node2;  // [vnodes] -> exists
...
//    for (auto i2 = query.itemFilter.begin(), ei2 = query.itemFilter.end(); i2 != ei2; i2++) {
//				// CHECK 用
//		    if ( cnt%100==0 ){ cerr << cnt << "/" << icnt << endl;}
//		    cnt++;
...
//        }
        ...
        if (itemFreq.find(*i2) == itemFreq.end()) {
            ...
            if (isTraGranu) {
  	        ...
  	        checked_node2[vnode2] = itemFreq[*i2];
  	        cerr << "i2chk " << node2 << " " << *i2 << " " << itemFreq[*i2] << endl; 

      	  } else {
        		itemFreq[*i2] = checked_node2[vnode2]; 
        		continue;
       ...
       // }
        ...
        itemInTheAtt2 = itemInTheAtt2 & query.itemFilter;
				...
				cerr << "runrun " <<  itemInTheAtt2.numberOfOnes()  << endl;
				// kokomo
      	Ewah tra_i2_tmp1;
        for (auto at2 = itemInTheAtt2.begin(), eat2 = itemInTheAtt2.end(); at2 != eat2; at2++) {
					if (isTimeOut) {stat = 2; break;}
        ...
        Ewah tra_i2 = tra_i2_tmp1 & tarTraBmp;
				...
				cerr << "runtra " << tra_i2.numberOfOnes() << endl;
        unordered_map<vector<string>, bool , boost::hash<vector<string>>> checked_tra1;
//memo traNum = mt_occ->countKeyValue(query.granularity.first, tarTraBmp);
        ...
        for (auto t2 = tra_i2.begin(), et2 = tra_i2.end(); t2 != et2; t2++) {
	        ...
	        checked_tra1[vTraAtt] = true;
    				...
    				//granuBmp.padWithZeroes(mt_occ->itemAtt->itemMax + 1);
    				//granuBmp.inplace_logicalnot();
    				//vector<string> traAttVal;
    				//mt_occ->traAtt->traNo2traAtt(*t2,  query.granularity.first, traAttVal);
				    //mt_occ->bmpList.GetVal(query.granularity.first, traAttVal, granuBmp);
  				  //Ewah zeroBmp;
				    
				   // for (auto xt2 = granuBmp.begin(), xet2 = granuBmp.end(); xt2 != xet2; xt2++) {
	         //   Ewah titem_i1 = mt_occ->occ[*t2] & query.itemFilter;
	         //   item_i1 = titem_i1 | item_i1;
				   // }
				  
					} else {
            item_i1 = mt_occ->occ[*t2] & query.itemFilter;
					}
					//cerr << "runi1 " << item_i1.numberOfOnes() << endl;
          //      for (auto i1 = item_i1.begin(), ei1 = item_i1.end(); i1 != ei1; i1++) {
          //          if (isTimeOut) {stat = 2; break;}
          //          if (*i1 >= *i2) break;
           //         coitems[*i1]++;
           //     }
            ...
            if (checked_node1.find({node1, traAtt}) == checked_node1.end()) {
            	checked_node1[{node1, traAtt}] = true;
						...
						//pair
						//cerr << "pair " << traAtt << " " << node2 << " " << node1 << endl;
						// itemNo(1)が持つ属性(vnode1)から、requestで指定された粒度で対象itemNoを拡張する(itemInTheAtt1)
						Ewah itemInTheAtt1 = mt_occ->itemAtt->bmpList.GetVal(query.granularity.second, vnode1);
						itemInTheAtt1 = itemInTheAtt1 & query.itemFilter;

            if (*(itemInTheAtt1.begin()) >= *(itemInTheAtt2.begin())) break;
      ...
      //  }
        ...
        for (auto i1 = coitems.begin(), ei1 = coitems.end(); i1 != ei1; i1++) {
            ...
            size_t freq = i1->second;
            float sup = (float)freq / traNum;
            cerr << "pair0 " << item1 << " " << item2  << " " << i1->second << " " << sup << " " << query.selCond.minSup << endl;
            ...
            res.insert(make_pair(skey,dt));
            cerr << "chksize "  << res.size() << " " << query.sendMax << endl;
            if (res.size() > query.sendMax) {
            	res.pop();
              stat = 1;
